...
public class ActionSettingAdapter extends ArrayAdapter {
    ArrayList<ActionItem> actionList;
    Context context;
    Integer[] data;
    PackageManager pm;

    public ActionSettingAdapter(final Context context, final int n, final ArrayList actionList) {
    ...
    class LoadImage extends AsyncTask {
        ...
        final ActionSettingAdapter this$0;

        public LoadImage(final ImageView imv) {
            super();
            this.this$0 = ActionSettingAdapter.this;
            this.imv = imv;
        }

        protected Drawable doInBackground(final String[] array) {
            try {
                return ActionSettingAdapter.this.pm.getApplicationIcon(array[0]);
            } catch (Exception ex) {
                return null;
            }
        }
